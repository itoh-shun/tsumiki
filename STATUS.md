...
- **インストール**（`tsumiki_0.1.0_x64-setup.exe` / `.msi`）。しなくても `app.exe` を
  直接起動すれば動作は同じ。スタートメニューに載るかどうかの違い
- **アプリの自動起動**。`tauri-plugin-autostart` は入っているが、有効化していない
- **サービスの自動起動**（systemd）。今はアプリが起動時に立ち上げるので、アプリを
  起動すれば繋がる。WSL2 自体が止まっているときは立ち上がらない
- **`uv tool install`** で `tsumiki` を素のコマンドとして使えるようにする
  （今は `cd service && uv run tsumiki`）

**いずれもユーザーの環境を変える操作なので、都度確認してから行う。**
