...
**v0.1 として作ると決めたものは全部できた。** ここから先は、使ってみて足りないものを
見つける段階。機能を先に足すより、実際に使い始める方が早い。

### 実機で確認したこと（2026-09-04・自動起動）

トレイメニューが4項目（表示 / 一時停止 / 自動起動 / 終了）になっていることを目視で確認。
**これは機械では確認できなかった唯一の点。** トレイメニューは native な Win32 ポップアップで
UI Automation から項目テキストを列挙できず、スクリーンショットは DPI 150% で信用できない。
機能面はレジストリの登録・削除で裏付け済み。

## 使い始めるための準備は済んでいる（2026-09-04）

- **インストール済み**: `%LOCALAPPDATA%\tsumiki\app.exe`。ユーザー単位・管理者権限なし。
  スタートメニューに `tsumiki.lnk`。アンインストールは「アプリと機能」から
- **`tsumiki` コマンド**: `uv tool install --editable` で `~/.local/bin` に3つ
  （`tsumiki` / `tsumiki-mcp` / `tsumiki-service`）。**editable なのでリポジトリの
  コードを直せば即反映される**。リポジトリを移動・削除すると壊れる点だけ注意
- **自動起動**: トレイメニューから切り替える。**既定は OFF**。インストールしただけでは
  自動起動しない
- MCP は `uv --directory .../service run tsumiki-mcp` のまま登録されている。
  素の `tsumiki-mcp` にすると起動が少し速くなるが、動いているので変えていない

### サービスの自動起動（systemd）は入れていない

今はアプリが起動時に `ensure_running` で立ち上げるので、アプリを起動すれば繋がる。
**WSL2 自体が止まっているときは立ち上がらない。** 常用してみて不便なら入れる。

### 作っていない機能（v0.1 のスコープ外と決めたもの）

サーバー側にはあるが UI から触れないもの、そもそも無いもの。**使ってみて要ると
分かってから作る。**

- タスクのタイトル・本文の編集（一覧からは状態変更と削除だけ）
- 期限（`due`）の表示と設定
- プロジェクト（`/projects` はあるが UI が無い）
- 検索・絞り込み（状態チップだけ）
- 一時停止の状態が捕捉小窓から見えない（トレイのチェックだけ）
